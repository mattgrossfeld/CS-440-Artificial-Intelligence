...
def genProb(wordBag, smoothing_parameter):
    ...
    unk = np.log(smoothing_parameter / (totalWords + smoothing_parameter * (len(wordBag.keys()) + 1))) # a / (n + a (V+ 1))
    
    for word in wordBag.keys():
        probList[word] = np.log((wordBag[word] + smoothing_parameter) / (totalWords + smoothing_parameter * (len(wordBag.keys()) + 1)))
...
def removeWords(wordBag, size):
    newBag = {}
    while len(newBag) <= size:
        minKey = min(wordBag.keys(), key=lambda k: wordBag[k])
        del wordBag[minKey]
    return wordBag
...
def naiveBayes(train_set, train_labels, dev_set, smoothing_parameter, pos_prior):
    ...
    for reviewIndex, review in enumerate(train_set):
        for word in review:
            if isWordTooCommon(word) == True:   # If word is too common, skip the word
                ...
                if train_labels[reviewIndex] == 1: # and len(posWordBag.keys()) < 5000: # Positive
                    if word not in posWordBag.keys():
                        posWordBag[word] = 1
                    else:
                        posWordBag[word] += 1
                elif train_labels[reviewIndex] == 0: # and len(negWordBag.keys()) < 5000:                              # Negative (train_labels[reviewIndex] != 1 then it must be 0)
    ...
    #logPosProb = np.log(posProbList)
    #logNegProb = np.log(negProbList)
    #logPosUNK = np.log(posUNK)
    #logNegUNK = np.log(negUNK)

    # Done with training. Now development with MAP
    dev_labels = []
    #count = 0
    for devRev in dev_set:
        reviewIsPos = np.log(pos_prior)
        reviewIsNeg = np.log(1 - pos_prior)
        for word in devRev:
            if isWordTooCommon(word) == True:   # If word is too common, skip the word
                continue
            else:
                ...
                if word in negProbList.keys():
                    ...
                    reviewIsNeg += negUNK
        #count += 1
        #print("REVIEWS LEFT: " + str(len(dev_set) - count))
