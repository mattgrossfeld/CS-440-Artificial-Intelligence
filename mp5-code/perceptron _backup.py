...
def classify(train_set, train_labels, dev_set, learning_rate,max_iter):
	...
	for epoch in range(max_iter):							# Go through the training data however many times max_iter is to refine data.
		...
		for imageIndex in range(train_set.shape[0]):			# Go through each image.
			result = 0
			for featureIndex in range(train_set.shape[1]):								# Go through each feature of the image.
				result += (weights[featureIndex] * train_set[imageIndex][featureIndex]) + bias
			result = np.sign(result)
			if result != signLabels[imageIndex]:										# If results not same, we update the weights and bias
				bias += (learning_rate * (signLabels[imageIndex] - result))
				for featureIndex in range(train_set.shape[1]):
					weights[featureIndex] = weights[featureIndex] + (learning_rate * signLabels[imageIndex] * train_set[imageIndex][featureIndex])
	
	predictedLabels = [0 for x in range(len(dev_set))] 
	for imageIndex in range(dev_set.shape[0]):
		result = 0
		for featureIndex in range(dev_set.shape[1]):
			result += (weights[featureIndex] * dev_set[imageIndex][featureIndex]) + bias
		result = np.sign(result)
		if result == 1:
			predictedLabels[imageIndex] = 1
		else:
			predictedLabels[imageIndex] = 0
