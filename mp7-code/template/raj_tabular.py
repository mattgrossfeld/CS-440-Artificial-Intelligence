...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        currentStates = [self.discretize(state) for state in states]        
        qvalues = [self.model[i] for i in current_states]
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        current_qvalue = self.model[state][action]

        # UPDATING Q VALUE TABLE
        self.model[state][action] += self.lr * (target - current_qvalue)


        return (target - current_qvalue)**2
