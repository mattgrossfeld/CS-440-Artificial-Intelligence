...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        qVals = [self.model[self.discretize(state)] for state in states]
        return qVals
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        if done == False:
            diff = reward + self.gamma * np.max(self.model[self.discretize(next_state)])
        else:
            diff = reward

        state = self.discretize(state)
        currentQVal = self.model[state][action]

        self.model[state][action] = self.model[state][action] + (self.lr * (diff - currentQVal))

        loss = (diff - currentQVal)**2
...
if __name__ == '__main__':
    ...
    policy = TabQPolicy(env, buckets=(1, 5, 10, 15), actionsize=actionsize, lr=args.lr, gamma=args.gamma)
