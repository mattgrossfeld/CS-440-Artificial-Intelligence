...
class NeuralNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for index, image in enumerate(x):
            ...
            yVal = torch.matmul(self.hiddenToOutput.weight, tan) + self.hiddenToOutput.bias
...
def fit(train_set,train_labels,dev_set,n_iter,batch_size=100):
    ...
    losses = []
    net = NeuralNet(0.001, loss_fn, train_set.size()[1], 2)         # The input size is the length of features (3072). Output size is different possible classes (2).     
